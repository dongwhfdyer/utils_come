...
# Lazy initialization of tokenizers to avoid circular imports
_tokenizers_cache = {}

def _get_tokenizer(tmodel):
    """Lazy initialization of tokenizers to avoid circular imports"""
    if tmodel not in _tokenizers_cache:
        # Import here to avoid circular import
        from laion_clap.local_model_config import require_model_path

        if tmodel == "bert":
            bert_path = require_model_path("bert-base-uncased")
            _tokenizers_cache["bert"] = BertTokenizer.from_pretrained(bert_path, local_files_only=True)
        elif tmodel == "roberta":
            roberta_path = require_model_path("roberta-base")
            _tokenizers_cache["roberta"] = RobertaTokenizer.from_pretrained(roberta_path, local_files_only=True)
        elif tmodel == "bart":
            bart_path = require_model_path("facebook/bart-base")
            _tokenizers_cache["bart"] = BartTokenizer.from_pretrained(bart_path, local_files_only=True)

    return _tokenizers_cache.get(tmodel)
...
def tokenizer(text, tmodel="roberta", max_length=77):
    ...
    if tmodel == "transformer":
        ...
        roberta_tok = _get_tokenizer("roberta")
        result = roberta_tok(
            text,
            padding="max_length",
            truncation=True,
