"""
DCASE2025 Task 2 Evaluator (Modified Interface Version)

MINIMAL MODIFICATIONS FROM ORIGINAL:
1. Changed interface to accept absolute paths instead of relative paths
2. Removed dependency on tools.plot_anm_score (optional visualization)
3. All metric calculations remain EXACTLY the same

Original: dcase2025_task2_evaluator-main/dcase2025_task2_evaluator.py
"""
import os
import sys
import csv
...
# Remove dependency on plot tool (optional feature)
try:
    sys.path.append('./')
...
def output_result(target_dir, machines, section_ids, result_dir, additional_result_dir, seed, tag, out_all=False):
    ...
    for machine_idx, target_machine in enumerate(machines):
        if HAS_PLOT_TOOL and AnmScoreFigData is not None:
        ...
        for section_id in section_ids:
            ...
            for domain in ['source', 'target']:
                ...
                if anm_score_figdata is not None:
...
##############################################################################
# NEW: Simplified interface function (MINIMAL MODIFICATION)
##############################################################################
def evaluate_with_absolute_paths(
    predictions_dir,
    ground_truth_data_dir,
    ground_truth_domain_dir,
    ground_truth_attributes_dir,
    result_dir,
    out_all=False
):
    """
    NEW FUNCTION: Simplified interface that accepts absolute paths

    This function wraps the original output_result() with absolute paths.
    ALL METRIC CALCULATIONS remain exactly the same - only interface changes.

    Args:
        predictions_dir: Absolute path to directory with anomaly_score_*.csv and decision_result_*.csv
        ground_truth_data_dir: Absolute path to ground_truth_data directory
        ground_truth_domain_dir: Absolute path to ground_truth_domain directory
        ground_truth_attributes_dir: Absolute path to ground_truth_attributes directory
        result_dir: Absolute path to output directory
        out_all: Whether to output additional results

    Returns:
        (status, official_score_df, auc_df, score_df, paper_official_score_df)

    Example:
        status, official_df, auc_df, score_df, paper_df = evaluate_with_absolute_paths(
            predictions_dir="/path/to/stage2_results/teams/baseline",
            ground_truth_data_dir="/path/to/evaluator/ground_truth_data",
            ground_truth_domain_dir="/path/to/evaluator/ground_truth_domain",
            ground_truth_attributes_dir="/path/to/evaluator/ground_truth_attributes",
            result_dir="/path/to/output"
        )
    """
    # MODIFICATION: Override global directory variables with absolute paths
    global GROUND_TRUTH_DATA_DIR, GROUND_TRUTH_DOMAIN_DIR, GROUND_TRUTH_ATTRIBUTES_DIR

    # Save original values
    orig_data_dir = GROUND_TRUTH_DATA_DIR
    orig_domain_dir = GROUND_TRUTH_DOMAIN_DIR
    orig_attr_dir = GROUND_TRUTH_ATTRIBUTES_DIR

    try:
        # Set to absolute paths
        GROUND_TRUTH_DATA_DIR = ground_truth_data_dir
        GROUND_TRUTH_DOMAIN_DIR = ground_truth_domain_dir
        GROUND_TRUTH_ATTRIBUTES_DIR = ground_truth_attributes_dir

        # Get machines and sections from ground truth
        machines = get_machines(load_dir=ground_truth_data_dir)
        section_ids = get_section_ids(target_dir=ground_truth_data_dir)

        # Create output directory
        Path(result_dir).mkdir(parents=True, exist_ok=True)
        additional_result_dir = str(Path(result_dir) / "additional_results")
        if out_all:
            Path(additional_result_dir).mkdir(parents=True, exist_ok=True)

        # Call original output_result() - NO CHANGES TO LOGIC
        ...
        GROUND_TRUTH_DATA_DIR = orig_data_dir
        GROUND_TRUTH_DOMAIN_DIR = orig_domain_dir
        GROUND_TRUTH_ATTRIBUTES_DIR = orig_attr_dir
