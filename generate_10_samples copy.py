...
1. SILENCE DETECTION (HIGHEST PRIORITY):
   - If silence_percentage > 5%, MUST explicitly mention silent periods/pauses in caption
   - If silence_percentage > 20%, describe as "significant silent periods" or "intermittent operation"
   - If silence_percentage > 40%, describe as "predominantly silent with brief active periods"
   - If num_active_regions > 1, mention "X distinct active segments separated by silence"

2. Per-band temporal peaks reveal bursts: If a band's max is >20 dB above its mean, that frequency range has significant bursts/transients
...
IMPORTANT: If the silence description indicates pauses or silent periods (>5% silence), you MUST mention this in the caption using natural language like "with pauses", "intermittent", "periodic with gaps", or "brief bursts separated by silence".
...
IMPORTANT: If silence_percentage > 5%, explicitly mention silent periods or pauses in the caption. Use phrases like "with pauses", "intermittent", "X active segments separated by silence", or "predominantly silent".
