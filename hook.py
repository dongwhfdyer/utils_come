...
class CLAP_Module(torch.nn.Module):
    def __init__(self, enable_fusion=False, device=None, amodel= 'HTSAT-tiny', tmodel='roberta') -> None:
        ...
        from laion_clap.local_model_config import require_model_path
        if device is None:
        ...
        # Initialize tokenizer with required local path
        roberta_path = require_model_path("roberta-base")
        self.tokenize = RobertaTokenizer.from_pretrained(roberta_path, local_files_only=True)
    ...
    def load_ckpt(self, ckpt = None, model_id = -1, verbose = True):
        ...
        ----------
        ckpt: str
            if ckpt is specified, the model will load this ckpt, otherwise the model will use local checkpoint files. \n
            For fusion model, it will use the 630k+audioset fusion model (id=3). For non-fusion model, it will use the 630k+audioset model (id=1).
        model_id:
            if model_id is specified, you can use our best ckpt, as:
                id = 0 --> 630k non-fusion ckpt \n
                id = 1 --> 630k+audioset non-fusion ckpt \n
                id = 2 --> 630k fusion ckpt \n
                id = 3 --> 630k+audioset fusion ckpt \n
            Note that if your model is specied as non-fusion model but you load a fusion model ckpt, you will face an error.
        """
        from clap_module.factory import load_state_dict

        # Local checkpoint file names
        checkpoint_names = [
            '630k-best.pt',
        ...
        if ckpt is not None:
            ...
            logging.info(f'Load our best checkpoint from local files.')
            if model_id == -1:
                model_id = 3 if self.enable_fusion else 1
            package_dir = os.path.dirname(os.path.realpath(__file__))
            weight_file_name = checkpoint_names[model_id]
            ckpt = os.path.join(package_dir, weight_file_name)
            if os.path.exists(ckpt):
                logging.info(f'The checkpoint is found locally: {ckpt}')
            else:
                raise FileNotFoundError(f'Checkpoint file not found: {ckpt}. Please ensure you have downloaded the checkpoint files to the local directory.')
