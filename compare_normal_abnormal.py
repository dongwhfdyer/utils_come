...
FEATURE_THRESHOLDS = {
    'spectral_centroid_mel': {
        'threshold': 10.0,  # mel bins
        'unit': 'mel bin',
        'description': 'Frequency center shift'
    },
    'temporal_energy_std': {
        'threshold': 10.0,  # dB
        'unit': 'dB',
        'description': 'Temporal variability change'
    },
    'stationarity': {
        'threshold': 0.2,  # unitless (0-1 scale)
        'unit': '',
        'description': 'Pattern stability change'
    },
    'energy_concentration': {
        'threshold': 3.0,  # ratio
        'unit': 'x',
        'description': 'Energy focus change'
    },
    'num_peaks': {
        'threshold': 3,  # count
        'unit': '',
        'description': 'Event count change'
    },
    'silence_percentage': {
        'threshold': 15.0,  # percentage points
        'unit': '%',
        'description': 'Silence pattern change'
    },
    'dominant_mel_bin': {
        'threshold': 10,  # mel bins
        'unit': 'mel bin',
        'description': 'Dominant frequency shift'
    },
    'spectral_spread_mel': {
        'threshold': 5.0,  # mel bins
        'unit': 'mel bins',
        'description': 'Frequency spread change'
    },
}

# Key features to always display
KEY_FEATURES = [
    'spectral_centroid_mel',
    'temporal_energy_std',
    'stationarity',
...
def format_feature_value(feature_name: str, value: float) -> str:
    ...
    if isinstance(value, int) or feature_name == 'num_peaks':
        formatted = f"{int(value)}"
    elif feature_name in ['stationarity', 'energy_concentration']:
